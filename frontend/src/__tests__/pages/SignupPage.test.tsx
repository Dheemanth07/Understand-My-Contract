...
jest.mock('@/components/GoogleSignInButton', () => ({
  GoogleSignInButton: function MockGoogleSignInButton({ children }: any) {
...
describe('SignupPage', () => {
  beforeEach(() => {
    ...
    mockSupabaseAuth.signUp.mockResolvedValue({
      data: { user: { id: 'new-user', identities: [{ id: 'new' }] }, session: null },
      error: null,
    });
  });
  ...
  it('shows success toast after signup', async () => {
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: "Signup successful!" }));
    });
  });
  ...
  it('shows error when account already exists', async () => {
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ description: "User already exists" }));
    });
  });
  ...
  it('displays error toast on signup failure', async () => {
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ description: errorMsg }));
    });
  });
