...
export const test = base.extend<{
    ...
    db: async ({}, use) => {
        const uri =
            ...
            "mongodb://127.0.0.1:27017/understand_my_contract_test";
        ...
        async function seedAnalysis(doc: any) {
            if (!mongoose.connection.readyState) await connect();
        ...
        async function getAnalysisById(id: any) {
            if (!mongoose.connection.readyState) await connect();
            const Analysis = mongoose.connection.collection("analyses");
        ...
        async function deleteAnalysisById(id: any) {
            if (!mongoose.connection.readyState) await connect();
            const Analysis = mongoose.connection.collection("analyses");
