...
describe("Router configuration", () => {
    ...
    it("protects /dashboard route with PrivateRoute", () => {
        const dashboardRoute = (router.routes as any[]).find(
            (route) => route.path === "/dashboard",
        );
        expect(
            dashboardRoute?.element?.type?.name ||
                dashboardRoute?.element?.props?.element?.type?.name,
        ).toBeDefined();
    });

    it("protects /history route with PrivateRoute", () => {
        const historyRoute = router.routes.find(
            (route: any) => route.path === "/history",
        );
        expect(historyRoute).toBeDefined();
    });

    it("protects /history/:id route with PrivateRoute", () => {
        const historyDetailRoute = router.routes.find(
            (route: any) => route.path === "/history/:id",
        );
    ...
    it("signin and signup routes are public (not protected)", () => {
        const signinRoute = (router.routes as any[]).find(
            (route) => route.path === "/signin",
        );
        const signupRoute = (router.routes as any[]).find(
